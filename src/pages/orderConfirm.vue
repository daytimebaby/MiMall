<template>
  <div class="order-confirm">
    ...
    <div class="wrapper">
      <div class="container">
        <div class="order-box">
          <div class="item-address">
            ...
            <div class="addr-list clearfix">
              <div class="addr-info" :class="{ 'checked': index == checkIndex }" @click="checkIndex = index"
                ...
                <div class="street">{{  item.receiverProvince + ' ' + item.receiverCity + ' ' + item.receiverDistrict +
          ...
          <div class="detail">
            <div class="item">
              <span class="item-name">商品件数：{{  count  }}</span>
              <span class="item-val">件</span>
            </div>
            <div class="item">
    ...
    <modal title="删除确定" btnType="1" :showModal="showDelModal" v-on:cancel="showDelModal = false"
      v-on:submit="submitAddress">
      <template v-slot:body>
        <p> 您确认要删除此地址么？</p>
      </template>
    </modal>
    <modal title="新增确认" btnType="3" :showModal="showEditModal" v-on:cancel="showEditModal = false"
      v-on:submit="submitAddress">
      <template v-slot:body>
        <div class="edit-wrap">
          <div class="item">
            <input type="text" class="input" placeholder="姓名" v-model="checkedItem.receiverName">
            <select name="receiverPhone" class="mobile-select" v-model="checkedItem.receiverPhone">
              <option value="010">010</option>
              <option value="020">020</option>
              <option value="030">030</option>
            </select>
            <input type="text" class="input phone-input" placeholder="手机号" v-model="checkedItem.receiverMobile">
          </div>
          <div class="item">
...
export default {
  ...
  data() {
    return {
      ...
      showDelModal: false,//是否显示删除
      showEditModal: false,//是否显示新增或者编辑弹框
      checkIndex: 0,//是否选择当前地址
    }
  },
  components: {
    OrderHeader,
